...
describe("syncMemoryWikiBridgeSources", () => {
  ...
  it("does not prune existing bridge pages when memory capability is unavailable", async () => {
    const workspaceDir = await createBridgeWorkspace("missing-capability-prune");
    const { rootDir: vaultDir, config } = await createVault({
      rootDir: nextCaseRoot("missing-capability-prune-vault"),
      config: {
        vaultMode: "bridge",
        bridge: {
          enabled: true,
  ...
  it.runIf(process.platform !== "win32")(
    "refuses bridge artifacts with symlinked parent directories",
    async () => {
