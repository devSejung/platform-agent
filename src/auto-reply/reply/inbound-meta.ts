...
export function buildInboundMetaSystemPrompt(
  ...
  const payload = {
    ...
    session_mode: isDirect ? "single-user" : "multi-user-shared",
    session_behavior: isDirect
      ? undefined
      : {
  ...
  return [
    ...
    !isDirect
      ? "For group/channel chats, assume multiple humans may address the same shared session and that the session may have been freshly initialized for this conversation."
      : "",
    !isDirect
      ? "Do not assume personal continuity for one sender across the whole session; use sender labels and recent room context to decide whether clarification is needed."
      : "",
    "",
    "```json",
